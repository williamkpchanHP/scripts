...
schkeyList = gsub("/", "", schkeyList) # different on diff sites
cat(red("\nTotal length(schkeyList) from pornpicBatch.txt: ",length(schkeyList), "\n"))
...
doBatchjob <- function(schkey){
  ...
  for(i in 1:length(addr)){
   ...
   if(i == 10){
     ...
     cat(red(
          ...
          "Expect total time required: ", dhms(ecTime),"\n\n"
        ))
   }
  }
  ...
  if(combinedMode){
    ...
    outFilename = paste0("combinedMode_", format(ProcessTime, "%m%d%H%M"), ".html")
