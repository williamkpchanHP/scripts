...
askforName <-function(){		# "abc def" -> "abc+def"
	Selection = readline(prompt="enter user name: ")
	if(Selection == "")   { 
...
url = paste0("https://xhamster.com/users/", userName, "/videos")
  ...
  if(lastLineNum>1){
    for (page in 2:lastLineNum){
         url = paste0("https://xhamster.com/users/", userName, "/videos/", page)
    	cat(" ", page, " url: ", url, "\n")
         pagesource <- read_html(url, warn=F, encoding = "UTF-8")
         assembleContents(pagesource)
    }
...
theFilename = paste0(userName, "User_xham.html")
