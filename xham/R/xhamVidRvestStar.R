...
askforName <-function(){		# "abc def" -> "abc+def"
	Selection = readline(prompt="enter star name: ")
	if(Selection == "")   { 
...
url = paste0("https://xhamster.com/pornstars/", channelName, "/")
  ...
  if(lastLineNum>1){
    for (page in 2:lastLineNum){
         url = paste0("https://xhamster.com/pornstars/", channelName, "/", page)
    	cat(" ", page, " url: ", url, "\n")
         pagesource <- read_html(url, warn=F, encoding = "UTF-8")
         assembleContents(pagesource)
    }
...
theFilename = paste0(channelName, "Star_xham.html")
