...
cat(pink("\ninput from xvideoURL.txt\n"))
theName = readLines("xvideoURL.txt")
...
boilPages <- function(theURL){
  ...
  pagesource = unlist(strsplit(pagesource, "var video_related"))
  pagesource = pagesource[2]
  pagesource = unlist(strsplit(pagesource, "</script>"))
  pagesource = pagesource[1]
  pagesource = unlist(strsplit(pagesource, 'u":'))
  lineIdx = grep("/video", pagesource)
  pagesource = pagesource[lineIdx]
  keywordList = character()
  for( i in pagesource){
    outputLine = paste0('<a href="https://www.xvideos.com', i)
# browser()
    outputLine = gsub('com./video', 'com/video', outputLine)
    outputLine = gsub(',"i":', '><img class="lazy" data-src=', outputLine)
    outputLine = gsub(',"c".*?tf":"', '><br>', outputLine )
    outputLine = gsub(',"t".*', '</a>', outputLine )
    keywordList = c(keywordList,  outputLine)
  }
  return(keywordList)
}
