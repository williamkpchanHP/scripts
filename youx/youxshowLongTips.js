...
function assembleLIne(keyElement) {
  for (i = 0; i < rows; i++) {
      keyElement = keyElement.replaceAll(keyMat[i][1], keyMat[i][0]);
  }
  keyElement = keyElement.replaceAll("_320x_", "")
  keyElement = keyElement.replaceAll("gthumb", "content")
...
function randomPos() {
  locpointer = Math.floor(Math.random() * document.body.scrollHeight);
  window.scrollTo(0, locpointer);
}
...
function toggle_automode() {
  if(automodeSwitch == false){
    autoInterval = setInterval(function(){ forwardInTip(); }, timeInterval);
